...
const DeleteModal = ({id, onCloseModal, title, nameToDelete }) => {
  ...
  return (
   ...
   <button onClick={onCloseModal} className="Frame36 hover:bg-gradient-to-l from-blue to-lightBlue hover:text-white smOnly:w-64 smOnly:h-10 w-32 h-10 px-5 py-2 rounded-3xl border-2 border-blue justify-center items-center gap-2 inline-flex  text-blue text-base font-bold font-['Manrope'] tracking-wide">
    ...
    <button onClick={onDelete} className="Frame35 smOnly:w-64 smOnly:h-10  md:ml-4 w-32 px-5 py-2 bg-blue hover:bg-gradient-to-l from-blue to-lightBlue  rounded-3xl justify-center items-center gap-2 inline-flex text-stone-50 text-base font-bold font-['Manrope'] tracking-wide">
     Yes<span>
     <Svg id={'icon-trash'} size={'24px'} stroke={'white'} fill={'#54ADFF'}/></span>
    </button>
   </div>
  </div>
  )
